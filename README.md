...
Production runs directly from the git checkout — all state (SQLite
databases, uploads, secrets) lives inside this directory, and the only
artifacts outside it are systemd units generated by `make install`.
Day-to-day:

```bash
make install    # one-time: apt deps + systemd units (uses sudo)
make update     # bundle → db:prepare → assets:precompile → restart
make status     # service + backup timer
make logs       # follow the journal
```

See [deploy/DEPLOYMENT.md](deploy/DEPLOYMENT.md) for the full runbook,
including the reverse-proxy setup (Caddy on a separate machine, serving the
app at the `/glove` subpath), S3 backups, and machine migration.
