#!/usr/bin/env bash
#
# Back up the Glove Rails app SQLite databases and uploaded files to S3.
#
...
#   S3_PREFIX        – key prefix inside the bucket  (default: "glove")
#   APP_ROOT         – path to the Rails app root (the repo root)
...
WORK_DIR=$(mktemp -d "/tmp/glove-backup-XXXXXX")
...
on_failure() {
  notify_ntfy "Glove backup failed" "Backup to s3://${S3_BUCKET}/${S3_PREFIX}/ failed at $(date -Iseconds)" "rotating_light,backup"
}
...
if [[ -d "${STORAGE_DIR}" ]]; then
  aws s3 sync "${STORAGE_DIR}" "s3://${S3_BUCKET}/${S3_PREFIX}/storage/${TIMESTAMP}/" \
    ...
    --exclude "*.sqlite3-shm" \
    --quiet
  echo "  Uploaded storage files"
fi

echo "[$(date -Iseconds)] Backup complete"

notify_ntfy "Glove backup succeeded" "Backup to s3://${S3_BUCKET}/${S3_PREFIX}/ completed at $(date -Iseconds)" "white_check_mark,backup"
